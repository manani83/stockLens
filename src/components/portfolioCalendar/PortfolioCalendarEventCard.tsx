...
type PortfolioCalendarEventCardProps = {
  event: PortfolioCalendarEvent;
};

export function PortfolioCalendarEventCard({
  event,
}: PortfolioCalendarEventCardProps) {
  return (
    <article className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
      ...
      {event.holdingInvestmentKrw ? (
        <p className="mt-3 text-xs text-slate-500">
          저장 투자금: {formatKrw(event.holdingInvestmentKrw)}
        </p>
      ) : null}
      {event.memo ? <p className="mt-1 text-xs text-slate-500">메모: {event.memo}</p> : null}
      {event.dataAsOf ? (
        <p className="mt-1 text-xs text-slate-500">데이터 기준일: {event.dataAsOf}</p>
      ) : null}
    </article>
  );
}
